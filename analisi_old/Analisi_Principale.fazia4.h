...
fillh("hnz32",(float)ival[index[0]],1);
 ...
 float vrel[3];
  ...
  if(evento.moltepl>1)
    {
fillh("hneventi",158.,1);
      if(evento.z[index[1]]>=5)
	{
fillh("hneventi",159.,1);
      int iselect=0;
 if((evento.z[index[0]]+evento.z[index[1]])>13)
			{
fillh("hneventi",160.,1);
if(Classe_analisi::Getanalisi()->reazione.zp==20 &&Classe_analisi::Getanalisi()->reazione.ap==40&& (evento.z[index[0]]+evento.z[index[1]])<22)
  {
    iselect=1;
  }

if(Classe_analisi::Getanalisi()->reazione.zp==20 &&Classe_analisi::Getanalisi()->reazione.ap==48 &&Classe_analisi::Getanalisi()->reazione.ebeamv<30&& (evento.z[index[0]]+evento.z[index[1]])<25)
  {
    iselect=1;
  }

if(Classe_analisi::Getanalisi()->reazione.zp==20 &&Classe_analisi::Getanalisi()->reazione.ap==48 &&Classe_analisi::Getanalisi()->reazione.ebeamv>30&& (evento.z[index[0]]+evento.z[index[1]])<23)
  {
    iselect=1;
  }
...
fillh("hneventi",161.,1);
  thetarel=Classe_formule::thetarel(vpcm[index[1]],vpcm[index[0]]);
		      Classe_formule::vrel(vpcm[index[1]],vpcm[index[0]],vrel);

		      fillh("hthetarelvrel_noselnocut",Classe_formule::modulo(vrel),thetarel,1);
		     

		      if(iselect==1)
			    {
fillh("hneventi",162.,1);
		      fillh("hthetarelvrel_nosel",Classe_formule::modulo(vrel),thetarel,1);
		      fillh("hthetarelvrel_norm",Classe_formule::modulo(vrel)/((float)evento.z[index[0]]*(float)evento.z[index[1]]),thetarel,1);

if(evento.rcoqf[index[1]]==1&&evento.rcoqf[index[0]]==1)
  {
fillh("hthetarelvrel_nosel_masse",Classe_formule::modulo(vrel),thetarel,1);
 fillh("hthetarelvrel_nosel_viola",Classe_formule::modulo(vrel)/viola((int)evento.z[index[0]],(int)evento.z[index[1]],(int)evento.a[index[0]],(int)evento.a[index[1]]),thetarel,1);
		      ...
		      if(Classe_formule::modulo(vrel)<7 && thetarel<30)
			{
			  //cout<<evento.z[index[0]]<<" "<<evento.z[index[1]]<<" "<<ztot<<" "<<ptotz<<" "<<evento.moltepl<<" "<<Classe_formule::modulo(vrel)<<" "<<thetarel<<" "<<Classe_analisi::Getanalisi()->nentry<<endl;
			  //cout<<vpcm[index[0]][0]<<" "<<vpcm[index[0]][1]<<" "<<vpcm[index[0]][2]<<" "<<vpcm[index[1]][0]<<" "<<vpcm[index[1]][1]<<" "<<vpcm[index[1]][2]<<" "<<evento.rcoqf[index[0]]<<" "<<evento.rcoqf[index[1]]<<endl;
			}
 if(Classe_analisi::Getanalisi()->tipo_analisi<200)
   {
     fillh("hb2bsel",mcevent.par_urto,1);
   }
			    }

 if(Classe_analisi::Getanalisi()->tipo_analisi<200)
   {
     fillh("hb2b",mcevent.par_urto,1);
   }
...
if(evento.rcoqf[index[1]]==1&&evento.rcoqf[index[0]]==1)
  {
    //  cout<<"qui"<<" "<<ivt<<endl;
	 ...
	 fillh("htkeindextot",tke,ivt,1);
	   ...
	   float qval=-Classe_analisi::Getanalisi()->Delta[(int)evento.z[index[0]]][(int)evento.a[index[0]]]-Classe_analisi::Getanalisi()->Delta[(int)evento.z[index[1]]][(int)evento.a[index[1]]]+Classe_analisi::Getanalisi()->Delta[(int)ztt][(int)att];
	   fillh("hqvalzt",ztt,qval,1);
	   fillh("hqvaletaz",etaz*10,qval,1);
	   fillh(Form("hqvalzt%d",ieta),ztt,qval,1);
	 ...
	 for(int k=0;k<3;k++)
	   {
	     vvplab[k]=((float)evento.a[index[0]]*vplab[index[0]][k]+(float)evento.a[index[1]]*vplab[index[1]][k])/(float)att;
	   }
	 float pcoppialab=(float)att*Classe_formule::modulo(vvplab)*Classe_formule::amu/Classe_formule::cluce;
fillh("hthetarelp",pcoppialab,thetarel,1);
 fillh("hthetarelpnorm",pcoppialab/pbeam,thetarel,1);
 // cout<<pcoppialab<<endl;
...
fillh("hocchigt90a_zslt120",vpars,vperps,1);
      ...
      if(evento.z[index[1]]<5&&evento.z[index[0]]>=5) //il controllo sul grosso aggiunto il 13 ottobre 2022
	{
fillh("hneventi",164.,1);
	  if(evento.rcoqf[index[0]]==1)
	    {
fillh("hneventi",165.,1);

 float pbiglab=(float)evento.a[index[0]]*Classe_formule::amu*sqrt(pow(vplab[index[0]][0],2)+pow(vplab[index[0]][1],2)+pow(vplab[index[0]][2],2))/Classe_formule::cluce;

 fillh("hzp",pbiglab,(float)evento.z[index[0]],1);
 fillh("hzpnorm",pbiglab/pbeam,(float)evento.z[index[0]],1);

 if(evento.z[index[0]]>=10)
   {
fillh("hneventi",170.,1);
...
if(Classe_analisi::Getanalisi()->tipo_analisi<200)
   {
     fillh("hb1b",mcevent.par_urto,1);
     if(evento.z[index[0]]>=10)
       {
fillh("hb1bzb10",mcevent.par_urto,1);
       }
   }
...
float vrel23[3]; 
   ...
   if(evento.moltepl>1 && evento.z[index[0]]>=10)
     {
       float vpar,vperp;
       if(m7Be==1)
	 {
	   
	   Classe_formule::vparvperp(vpcm[j7Be[0]],vpcm[index[0]],&vpar,&vperp);
	   fillh("hocchi7Be",vpar,vperp,1);
	 } 
       if(m9Be==1)
	 {
	   
	   Classe_formule::vparvperp(vpcm[j9Be[0]],vpcm[index[0]],&vpar,&vperp);
	   fillh("hocchi9Be",vpar,vperp,1);
	 } 
       if(m10Be==1)
	 {
	   
	   Classe_formule::vparvperp(vpcm[j10Be[0]],vpcm[index[0]],&vpar,&vperp);
	   fillh("hocchi10Be",vpar,vperp,1);
	 } 

       if(m11C==1)
	 {
	   
	   Classe_formule::vparvperp(vpcm[j11C[0]],vpcm[index[0]],&vpar,&vperp);
	   fillh("hocchi11Cstab",vpar,vperp,1);
	   float vpar3,vperp3,vpar3cm,vperp3cm,vrec;
 Classe_formule::vparvperprecoil(vpcm[j11C[0]],vpcm[index[0]],evento.a[j11C[0]],evento.a[index[0]],&vpar3,&vperp3,&vpar3cm,&vperp3cm,&vrec);
fillh("hocchi11Cstabrecoil",vpar3,vperp3,1);
fillh("hocchi11Cstabrecoil2",vpar3cm,vperp3cm,1);
fillh("hocchi11Cstabrecoil3",vpar3+vrec,vperp3,1);


	   float vrel11C[3];
		Classe_formule::vrel(vpcm[j11C[0]],vpcm[index[0]],vrel11C);
		float thetarel11C=Classe_formule::thetarel(vpcm[j11C[0]],vpcm[index[0]]);
		fillh("hthetarelvrel11Cstab",Classe_formule::modulo(vrel11C),thetarel11C,1);
      ...
      if(m12C==1)
	 {
	   ...
	   Classe_formule::vparvperp(vpcm[j12C[0]],vpcm[index[0]],&vpar,&vperp);
	   float vpar3,vperp3,vpar3cm,vperp3cm,vrec;
 Classe_formule::vparvperprecoil(vpcm[j12C[0]],vpcm[index[0]],evento.a[j12C[0]],evento.a[index[0]],&vpar3,&vperp3,&vpar3cm,&vperp3cm,&vrec);
	   fillh("hocchi12Cstab",vpar,vperp,1);
	   fillh("hocchi12Cstabrecoil",vpar3,vperp3,1);
	   fillh("hocchi12Cstabrecoil2",vpar3cm,vperp3cm,1);
	   fillh("hocchi12Cstabrecoil3",vpar3+vrec,vperp3,1);

	   float vrel12C[3];
		Classe_formule::vrel(vpcm[j12C[0]],vpcm[index[0]],vrel12C);
		float thetarel12C=Classe_formule::thetarel(vpcm[j12C[0]],vpcm[index[0]]);
		fillh("hthetarelvrel12Cstab",Classe_formule::modulo(vrel12C),thetarel12C,1);
      ...
      if(m13C==1)
	 {
	   ...
	   Classe_formule::vparvperp(vpcm[j13C[0]],vpcm[index[0]],&vpar,&vperp);
	   fillh("hocchi13Cstab",vpar,vperp,1);
	   float vpar3,vperp3,vpar3cm,vperp3cm,vrec;
 Classe_formule::vparvperprecoil(vpcm[j13C[0]],vpcm[index[0]],evento.a[j13C[0]],evento.a[index[0]],&vpar3,&vperp3,&vpar3cm,&vperp3cm,&vrec);
fillh("hocchi13Cstabrecoil",vpar3,vperp3,1);
fillh("hocchi13Cstabrecoil2",vpar3cm,vperp3cm,1);
fillh("hocchi13Cstabrecoil3",vpar3+vrec,vperp3,1);

	   float vrel13C[3];
		Classe_formule::vrel(vpcm[j13C[0]],vpcm[index[0]],vrel13C);
		float thetarel13C=Classe_formule::thetarel(vpcm[j13C[0]],vpcm[index[0]]);
		fillh("hthetarelvrel13Cstab",Classe_formule::modulo(vrel13C),thetarel13C,1);
	    ...
	    if(imp>=0)
	      {
		     ...
		     float Q=Classe_analisi::Getanalisi()->Delta[5][9]-Classe_analisi::Getanalisi()->Delta[4][8]-Classe_analisi::Getanalisi()->Delta[1][1];
		 ...
		 if(ja8Bemin>=0)
		   {
		     ...
		     float Q=Classe_analisi::Getanalisi()->Delta[6][12]-Classe_analisi::Getanalisi()->Delta[4][8]-Classe_analisi::Getanalisi()->Delta[2][4];
	...
	if(evento.z[index[0]]>=10)
	  {
	    float vpar,vperp;
	     ...
	     float vpar3,vperp3,vpar3cm,vperp3cm,vrec;
	     float a8=8;
	     Classe_formule::vparvperprecoil(vcmc,vpcm[index[0]],a8,evento.a[index[0]],&vpar3,&vperp3,&vpar3cm,&vperp3cm,&vrec);

	     
	     fillh(Form("h%d",200+(int)evento.z[index[0]]),vpar,erelmin,1);
	     fillh("h200",vpar,erelmin,1);
	     fillh("h1200",vpar3,erelmin,1);
	     fillh("h1200_2",vpar3cm,erelmin,1);
	     fillh("h1200_3",vpar3+vrec,erelmin,1);
	     ...
	     fillh("h1300",vpar3,vperp3,1);
	     fillh("h1300_2",vpar3cm,vperp3cm,1);
	     fillh("h1300_3",vpar3+vrec,vperp3,1);
 ...
 Classe_formule::vparvperprecoil(vpcm[ja],vpcm[index[0]],evento.a[ja],evento.a[index[0]],&vpar3,&vperp3,&vpar3cm,&vperp3cm,&vrec);		     
		     fillh("h1401",vpar3,vperp3,1);
		     fillh("h1401_2",vpar3cm,vperp3cm,1);
		     fillh("h1401_3",vpar3+vrec,vperp3,1);
...
fillh("hBea_erel",erelC,1);
 ...
 // float vpar3,vperp3;
 float a12=12;
 Classe_formule::vparvperprecoil(VC12,vpcm[index[0]],a12,evento.a[index[0]],&vpar3,&vperp3,&vpar3cm,&vperp3cm,&vrec);

fillh("h400",vpar2,vperp2,1);
fillh("h1400",vpar3,vperp3,1);
fillh("h1400_2",vpar3cm,vperp3cm,1);
fillh("h1400_3",vpar3+vrec,vperp3,1);

fillh("h405",vpar2,erelC-Q,1);
fillh("h1405",vpar3,erelC-Q,1);
fillh("h1405_2",vpar3cm,erelC-Q,1);
fillh("h1405_3",vpar3+vrec,erelC-Q,1);
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
 ...
 mixatore(indice,2,1,2,4,1,1,2);
	float Q=Classe_analisi::Getanalisi()->Delta[3][6]-Classe_analisi::Getanalisi()->Delta[1][2]-Classe_analisi::Getanalisi()->Delta[2][4];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
		float mu=2*4/6.;
...
mixatore(indice,3,1,2,4,1,3,6);
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[5][10]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[3][6];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m7Li>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
	float Q=Classe_analisi::Getanalisi()->Delta[5][11]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[3][7];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m16O>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[10][20]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[8][16];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m12C>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
	float Q=Classe_analisi::Getanalisi()->Delta[8][16]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[6][12];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m20Ne>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
	float Q=Classe_analisi::Getanalisi()->Delta[12][24]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[10][20];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m24Mg>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[14][28]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[12][24];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m28Si>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
	float Q=Classe_analisi::Getanalisi()->Delta[16][32]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[14][28];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m32S>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[18][36]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[16][32];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m36Ar>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[20][40]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[18][36];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(mp>=1 && m27Al>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
	float Q=Classe_analisi::Getanalisi()->Delta[14][28]-Classe_analisi::Getanalisi()->Delta[1][1]-Classe_analisi::Getanalisi()->Delta[13][27];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m40Ca>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[22][44]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[20][40];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(mp>=1 && m40Ca>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[21][41]-Classe_analisi::Getanalisi()->Delta[1][1]-Classe_analisi::Getanalisi()->Delta[20][40];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m48Ca>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[22][52]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[20][48];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(mp>=1 && m48Ca>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[21][49]-Classe_analisi::Getanalisi()->Delta[1][1]-Classe_analisi::Getanalisi()->Delta[20][48];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m7Be>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[6][11]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[4][7];
 float estarC11=0;
 int n11C=0;
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
if(evento.z[index[0]]>=10)
	...
	if(estarC11<15)
	  {
		...
		int i4=mixing.Prel.at(indice).at(i).at(4);
 ...
 float vpar3,vperp3,vpar3cm,vperp3cm,vrec;
 float a11=11;
 Classe_formule::vparvperprecoil(v11C,vpcm[index[0]],a11,evento.a[index[0]],&vpar3,&vperp3,&vpar3cm,&vperp3cm,&vrec);

		fillh("hocchi11C",vpar,vperp,1);
		fillh("hocchi11Crecoil",vpar3,vperp3,1);
		fillh("hocchi11Crecoil_2",vpar3cm,vperp3cm,1);
		fillh("hocchi11Crecoil_3",vpar3+vrec,vperp3,1);
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
		...
		//******** IN CORSO
	      if(evento.z[index[0]]>=10)
	      {
		fillh("h11C_estarmixZ10",erel-Q,1);
	      }  
	...
	float v11Cmix[3];
		...
		for(int kk=0;kk<3;kk++)
		  {
		    v11Cmix[kk]=mixing.Vcm_mix.at(indice).at(i).at(kk);
		    if(kk==2)v11Cmix[kk]=v11Cmix[kk]-Classe_analisi::Getanalisi()->reazione.vcm;
		  }

		Classe_formule::vparvperp(v11Cmix,vpcm[index[0]],&vpar,&vperp);
		fillh("hocchi11Cmix",vpar,vperp,1);
	float vrel11Cmix[3];
		Classe_formule::vrel(v11Cmix,vpcm[index[0]],vrel11Cmix);
		float thetarel11Cmix=Classe_formule::thetarel(v11Cmix,vpcm[index[0]]);
		fillh("hthetarelvrel11Cmix",Classe_formule::modulo(vrel11Cmix),thetarel11Cmix,1);
		fillh("hvz11Cmix",v11Cmix[2],1);
      ...
      if(malpha>=1 && m9Be>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[6][13]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[4][9];
 float estar13C=0;
 int n13C=0;
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vpar3,vperp3,vpar3cm,vperp3cm,vrec;
 float a13=13;
 Classe_formule::vparvperprecoil(v13C,vpcm[index[0]],a13,evento.a[index[0]],&vpar3,&vperp3,&vpar3cm,&vperp3cm,&vrec);
		Classe_formule::vparvperp(v13C,vpcm[index[0]],&vpar,&vperp);
		fillh("hocchi13C",vpar,vperp,1);
		fillh("hocchi13Crecoil",vpar3,vperp3,1);
		fillh("hocchi13Crecoil_2",vpar3cm,vperp3cm,1);
		fillh("hocchi13Crecoil_3",vpar3+vrec,vperp3,1);
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
		...
		//******** IN CORSO
	      if(evento.z[index[0]]>=10)
	      {
		fillh("h13C_estarmixZ10",erel-Q,1);
	      }  
	...
	float v13Cmix[3];
		...
		for(int kk=0;kk<3;kk++)
		  {
		    v13Cmix[kk]=mixing.Vcm_mix.at(indice).at(i).at(kk);
		    if(kk==2)v13Cmix[kk]=v13Cmix[kk]-Classe_analisi::Getanalisi()->reazione.vcm;
		  }

		Classe_formule::vparvperp(v13Cmix,vpcm[index[0]],&vpar,&vperp);
		fillh("hocchi13Cmix",vpar,vperp,1);
	float vrel13Cmix[3];
		Classe_formule::vrel(v13Cmix,vpcm[index[0]],vrel13Cmix);
		float thetarel13Cmix=Classe_formule::thetarel(v13Cmix,vpcm[index[0]]);
		fillh("hthetarelvrel13Cmix",Classe_formule::modulo(vrel13Cmix),thetarel13Cmix,1);
		fillh("hvz13Cmix",v13Cmix[2],1);
      ...
      if(malpha>=1 && m10Be>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[6][14]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[4][10];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m6He>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[4][10]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[2][6];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && mp>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[3][5]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[1][1];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(m3He>=1 && md>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[3][5]-Classe_analisi::Getanalisi()->Delta[2][3]-Classe_analisi::Getanalisi()->Delta[1][2];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(m9Be>=1 && mp>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[5][10]-Classe_analisi::Getanalisi()->Delta[1][1]-Classe_analisi::Getanalisi()->Delta[4][9];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(m3He>=1 && mp>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[3][4]-Classe_analisi::Getanalisi()->Delta[2][3]-Classe_analisi::Getanalisi()->Delta[1][1];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(m7Li>=1 && md>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[4][9]-Classe_analisi::Getanalisi()->Delta[3][7]-Classe_analisi::Getanalisi()->Delta[1][2];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(m6He>=1 && mt>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[3][9]-Classe_analisi::Getanalisi()->Delta[2][6]-Classe_analisi::Getanalisi()->Delta[1][3];
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(m14N>=1 && mp>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[8][15]-Classe_analisi::Getanalisi()->Delta[1][1]-Classe_analisi::Getanalisi()->Delta[7][14];
		float mu=14*1/15.;
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
     ...
     if(m13C>=1 && mp>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[7][14]-Classe_analisi::Getanalisi()->Delta[1][1]-Classe_analisi::Getanalisi()->Delta[6][13];
		float mu=13*1/14.;
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
     ...
     if(m12B>=1 && mp>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[6][13]-Classe_analisi::Getanalisi()->Delta[1][1]-Classe_analisi::Getanalisi()->Delta[5][12];
		float mu=12*1/13.;
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
     ...
     if(m11B>=1 && mp>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[6][12]-Classe_analisi::Getanalisi()->Delta[1][1]-Classe_analisi::Getanalisi()->Delta[5][11];
		float mu=11*1/12.;
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
     ...
     if(m10B>=1 && mp>=1)
	{
	  ...
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[6][11]-Classe_analisi::Getanalisi()->Delta[1][1]-Classe_analisi::Getanalisi()->Delta[5][10];
		float mu=10*1/11.;
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
      ...
      if(malpha>=1 && m9Be>=1 && evento.z[index[0]]>=10)
	{
	  for(int k=0;k<malpha;k++)
	    {
	      evento.isformix[jalpha[k]]=32;
	     
	    }
	  for(int k=0;k<m9Be;k++)
	    {
	      evento.isformix[j9Be[k]]=32;
	     
	    }

	  int indice=31;
	  mixatore(indice,32,1,2,4,1,4,9);
	  float erelmin=1000;
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[6][13]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[4][9];
 float estar13C=0;
	...
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
		...
		float erel=0.5*mu*Classe_formule::amu*pow(vvrel/Classe_formule::cluce,2); //- Qval (con il segno) per estar
		fillh("h9Bea_erelZ10bis",erel,1);
		
		fillh("h13C_estarZ10bis",erel-Q,1);
		estar13C=erel-Q;
	...
	if(estar13C<15)
	  {
		...
		Classe_formule::vparvperp(v13C,vpcm[index[0]],&vpar,&vperp);
		fillh("hocchi13CZ10bis",vpar,vperp,1);
		float vrel13C[3];
		Classe_formule::vrel(v13C,vpcm[index[0]],vrel13C);
		float thetarel13C=Classe_formule::thetarel(v13C,vpcm[index[0]]);
		fillh("hthetarelvrel13CZ10bis",Classe_formule::modulo(vrel13C),thetarel13C,1);
		fillh("hvz13CZ10bis",v13C[2],1);
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
		...
		fillh("h9Bea_emixZ10bis",erel,1);
		fillh("h13C_estarmixZ10bis",erel-Q,1);
		//******** IN CORSO
	...
	float v13Cmix[3];
		...
		Classe_formule::vparvperp(v13Cmix,vpcm[index[0]],&vpar,&vperp);
		fillh("hocchi13CmixZ10bis",vpar,vperp,1);
	float vrel13Cmix[3];
		Classe_formule::vrel(v13Cmix,vpcm[index[0]],vrel13Cmix);
		float thetarel13Cmix=Classe_formule::thetarel(v13Cmix,vpcm[index[0]]);
		fillh("hthetarelvrel13CmixZ10bis",Classe_formule::modulo(vrel13Cmix),thetarel13Cmix,1);
		fillh("hvz13CmixZ10bis",v13Cmix[2],1);
      ...
      if(malpha>=1 && m7Be>=1 && evento.z[index[0]]>=10)
	{
	  for(int k=0;k<malpha;k++)
	    {
	      evento.isformix[jalpha[k]]=33;
	     
	    }
	  for(int k=0;k<m7Be;k++)
	    {
	      evento.isformix[j7Be[k]]=33;
	     
	    }

	  int indice=32;
	  mixatore(indice,33,1,2,4,1,4,7);
	  float erelmin=1000;
	  int imin=-1;
	  float vcmc[3];
float Q=Classe_analisi::Getanalisi()->Delta[6][11]-Classe_analisi::Getanalisi()->Delta[2][4]-Classe_analisi::Getanalisi()->Delta[4][7];
 float estar11C=0;
	...
	for(UInt_t i=0; i<mixing.Prel.at(indice).size(); i++){
		...
		float erel=0.5*mu*Classe_formule::amu*pow(vvrel/Classe_formule::cluce,2); //- Qval (con il segno) per estar
		fillh("h7Bea_erelZ10bis",erel,1);
		
		fillh("h11C_estarZ10bis",erel-Q,1);
		estar11C=erel-Q;
	...
	if(estar11C<15)
	  {
		...
		Classe_formule::vparvperp(v11C,vpcm[index[0]],&vpar,&vperp);
		fillh("hocchi11CZ10bis",vpar,vperp,1);
		float vrel11C[3];
		Classe_formule::vrel(v11C,vpcm[index[0]],vrel11C);
		float thetarel11C=Classe_formule::thetarel(v11C,vpcm[index[0]]);
		fillh("hthetarelvrel11CZ10bis",Classe_formule::modulo(vrel11C),thetarel11C,1);
		fillh("hvz11CZ10bis",v11C[2],1);
...
float vvcm=Classe_formule::modulo(mixing.Vcm_mix.at(indice).at(i));
		...
		fillh("h7Bea_emixZ10bis",erel,1);
		fillh("h11C_estarmixZ10bis",erel-Q,1);
		//******** IN CORSO
	...
	float v11Cmix[3];
		...
		Classe_formule::vparvperp(v11Cmix,vpcm[index[0]],&vpar,&vperp);
		fillh("hocchi11CmixZ10bis",vpar,vperp,1);
	float vrel11Cmix[3];
		Classe_formule::vrel(v11Cmix,vpcm[index[0]],vrel11Cmix);
		float thetarel11Cmix=Classe_formule::thetarel(v11Cmix,vpcm[index[0]]);
		fillh("hthetarelvrel11CmixZ10bis",Classe_formule::modulo(vrel11Cmix),thetarel11Cmix,1);
		fillh("hvz11CmixZ10bis",v11Cmix[2],1);

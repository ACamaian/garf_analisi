...
void Classe_evento::Leggievento()
{
 ...
 //Convenzione tipo_analisi: 0-9 simulazioni 4p; 10-19 le stesse simulazioni geo; 20 exp human readable; 21 exp compatto  
  if(Classe_analisi::Getanalisi()->tipo_analisi<20) //MC geo o 4pi
    {
      if(Classe_analisi::Getanalisi()->tipo_analisi==0 || Classe_analisi::Getanalisi()->tipo_analisi==10)//Gemini 4p (0) o geo (10)
	{
 ...
 //fine per GeminiDIC
      ...
      if(Classe_analisi::Getanalisi()->tipo_analisi==1||Classe_analisi::Getanalisi()->tipo_analisi==11)//Hipse 4p (1) o geo (11)
	{
      ...
      if(Classe_analisi::Getanalisi()->tipo_analisi==2||Classe_analisi::Getanalisi()->tipo_analisi==12)//Baiocco 4p (2) o geo (12)
	{
...
Classe_analisi::Getanalisi()->ch->SetBranchAddress("vz",mcevent.vzprimamd);
 ...
 if(Classe_analisi::Getanalisi()->tipo_analisi==5||Classe_analisi::Getanalisi()->tipo_analisi==15)
{
  ...
  if(Classe_analisi::Getanalisi()->ch->GetBranch("Ameasured")!=0) Classe_analisi::Getanalisi()->ch->SetBranchAddress("Ameasured",mcevent.Ameasured);
      ...
      if(Classe_analisi::Getanalisi()->tipo_analisi==4 || Classe_analisi::Getanalisi()->tipo_analisi==14)//twingo+ gemini++
	{
...
//  Classe_analisi::Getanalisi()->ch->SetBranchAddress("Pzcmfragprimari",mcevent.pzprim);
      ...
      if(Classe_analisi::Getanalisi()->tipo_analisi==6 || Classe_analisi::Getanalisi()->tipo_analisi==16)//twingo+geminif90
	{
Classe_analisi::Getanalisi()->ch->SetBranchAddress("nev",&mcevent.nevorig);
      Classe_analisi::Getanalisi()->ch->SetBranchAddress("moltsec",&mcevent.moltepl);
Classe_analisi::Getanalisi()->ch->SetBranchAddress("b",&mcevent.par_urto);
...
Classe_analisi::Getanalisi()->ch->SetBranchAddress("estarprim",mcevent.estar_primari);
 Classe_analisi::Getanalisi()->ch->SetBranchAddress("j",mcevent.spin_primari);
     ...
     if(Classe_analisi::Getanalisi()->tipo_analisi==8 || Classe_analisi::Getanalisi()->tipo_analisi==18)//Langevin4D
       {
  ...
  if(Classe_analisi::Getanalisi()->tipo_analisi==20)//Exp LETTURA DI ODIE FORMA LEGGIBILE
    {
...
Classe_analisi::Getanalisi()->ch->SetBranchAddress(Form("hector_phosbox%d_phos%d_phosid_z",ip+1,ih+1),&expevent.phos_z[ip][ih]);
 ...
 //Classe_analisi::Getanalisi()->ch->SetBranchAddress(Form("hector_phosbox%d_phos%d_phosid_ga",ip+1,ih+1),&expevent.phos_ga[ip][ih]); //NOn ci sono entuple non compatte con ga dentro
  ...
  if(Classe_analisi::Getanalisi()->tipo_analisi==21)//Exp Lettura di ODIE Forma Compatta
...
Classe_analisi::Getanalisi()->ch->SetBranchAddress("event_num",&expcomp.event_num);
  ...
  if(Classe_analisi::Getanalisi()->tipo_analisi==22)//Exp Lettura di Ntupla Bologna
    {
...
Classe_analisi::Getanalisi()->ch->SetBranchAddress("trg_1",expbo.trigger);
  ...
  if(Classe_analisi::Getanalisi()->tipo_analisi==23)//Exp Lettura albero Fazietto da Kaliveda
    {
...
Classe_analisi::Getanalisi()->ch->SetBranchAddress("phi",&faziakali.phi);
  ...
  if(Classe_analisi::Getanalisi()->tipo_analisi==24)//Exp lettura tree di INDRAFAZIA (output kaliveda)
    {
...
bool Classe_evento::CaricaEvento(){
	Classe_analisi::Getanalisi()->ch->GetEntry(Classe_analisi::Getanalisi()->nentry);
	return true;
}
...
void Classe_evento::AnalizzaEvento()
{
  ...
  if(Classe_analisi::Getanalisi()->tipo_analisi<20)
    {
      if(Classe_analisi::Getanalisi()->tipo_analisi==1 || Classe_analisi::Getanalisi()->tipo_analisi==11)//HIPSE
	{
	  mcevent.fissione=-1;
      ...
      if(Classe_analisi::Getanalisi()->tipo_analisi==2 || Classe_analisi::Getanalisi()->tipo_analisi==12)//BAIOCCO
	{
	  mcevent.fissione=-1;
      ...
      // if(Classe_analisi::Getanalisi()->tipo_analisi==0 || Classe_analisi::Getanalisi()->tipo_analisi==10||Classe_analisi::Getanalisi()->tipo_analisi==4||Classe_analisi::Getanalisi()->tipo_analisi==14)//Gemini 
      // 	{
      // 		for(unsigned j=0;j<mcevent.moltepl;j++)
      // 	    {
      // 	      if(TMath::Nint(mcevent.z[j])==0 &&TMath::Nint(mcevent.a[j])==1)
      ...
      // 	    }
      // 	}//Gemini
     if(Classe_analisi::Getanalisi()->tipo_analisi==3 || Classe_analisi::Getanalisi()->tipo_analisi==13||Classe_analisi::Getanalisi()->tipo_analisi==4 || Classe_analisi::Getanalisi()->tipo_analisi==14||Classe_analisi::Getanalisi()->tipo_analisi==5 || Classe_analisi::Getanalisi()->tipo_analisi==15||Classe_analisi::Getanalisi()->tipo_analisi==6 || Classe_analisi::Getanalisi()->tipo_analisi==16||Classe_analisi::Getanalisi()->tipo_analisi==7 || Classe_analisi::Getanalisi()->tipo_analisi==17||Classe_analisi::Getanalisi()->tipo_analisi==9 || Classe_analisi::Getanalisi()->tipo_analisi==19)//AMD (3-13, 5-15, 7-17) oppure twingo+geminif90 oppure twingo+gemini++ oppure blob
	{
	  mcevent.fissione=-1;
	  mcevent.CN=-1;
	  mcevent.spin=-1;
      ...
      // printf("Moltepl=%d\n",mcevent.moltepl);
     if(Classe_analisi::Getanalisi()->tipo_analisi!=8 && Classe_analisi::Getanalisi()->tipo_analisi!=18)
       {
     for(unsigned j=0;j<mcevent.moltepl;j++)
    {
...
mcevent.philab[mcevent.moltcharged+mcevent.moltgamma]=57.296*TMath::ATan2(mcevent.vysecamd[j],mcevent.vxsecamd[j]);
 ...
 mcevent.vpartlab_cart[mcevent.moltcharged+mcevent.moltgamma][0]=Classe_formule::cluce*TMath::Sin(mcevent.thetalab[mcevent.moltcharged+mcevent.moltgamma]/57.296)*TMath::Cos(mcevent.phicm[mcevent.moltcharged+mcevent.moltgamma]/57.296);
  ...
  if(Classe_analisi::Getanalisi()->tipo_analisi<10){NoeffettiExp();}//mcarlo 4p

  if(Classe_analisi::Getanalisi()->tipo_analisi>=10 &&Classe_analisi::Getanalisi()->tipo_analisi<20 ) //mcarlo geo
      {
 //**********************
...
Classe_formule::polcar(mcevent.vpartcmexp[j],mcevent.thetacmexp[j],mcevent.phicmexp[j],mcevent.vpartcm_cartexp[j]);
  ...
  //**********************ODIE COMPATTO***********************
  if(Classe_analisi::Getanalisi()->tipo_analisi==21)//ODIE COMPATTO
    {
...
// 		  //il theta e il phi si assegnano con la spalmatura
 ...
 if(expcomp.value_worker_class_code[j]==1001) { //raw RCO strip
    ...
    //**********************NTUPLA di BOLOGNA***********************
  if(Classe_analisi::Getanalisi()->tipo_analisi==22)//NTUPLA di BOLOGNA
    {
      expevent.trig=0;
      for(int j=0;j<8;j++)
	{
  ...
  if(Classe_analisi::Getanalisi()->tipo_analisi==23)//NTUPLA di KALIVEDA per ISOFAZIA
    {
      expevent.trig=-1;
    ...
    //**********************NTUPLA DI KALIVEDA PER INDRAFAZIA***********************
  if(Classe_analisi::Getanalisi()->tipo_analisi==24)//NTUPLA di KALIVEDA per INDRAFAZIA
    {
      expevent.trig=-1;
...
if(Classe_analisi::Getanalisi()->tipo_analisi>=10 &&Classe_analisi::Getanalisi()->tipo_analisi<20 ) //mcarlo geo Si buttano i multiple hits
  {
...
{
  //Clear vectors in evento
  evento.moltepl=0;
  evento.z.clear();
  evento.a.clear();
  evento.epartcm.clear();
  evento.thetacm.clear();
  evento.phicm.clear();
  evento.vpartcm.clear();
  evento.vpartcm_x.clear();
  evento.vpartcm_y.clear();
  evento.vpartcm_z.clear();
  evento.vpcm.clear();
  evento.epartlab.clear();
  evento.thetalab.clear();
  evento.philab.clear();
  evento.vpartlab.clear();
  evento.vpartlab_x.clear();
  evento.vpartlab_y.clear();
  evento.vpartlab_z.clear();
  evento.vplab.clear();
  evento.indice_originale.clear();//solo per montecarlo 
  evento.tvolo.clear();//solo per sperimentale
  evento.tbunch.clear();//tempo per trovare il bunch riportato
  evento.codphos.clear();//solo per phoswich mc ed exp 1 gatof, 2 gagb, 3 gbgc
  evento.phosqf.clear();//solo per sperimentale phoswich 100 per id da gate, altro per linee Z
  evento.coderiv.clear(); // per i phos box*10+phos (parte da 1); per garfield isec*10+cesio+1000; BaF2 is 10000+nbaf
  evento.phosga.clear();// ga (solo per i phos exp)
  evento.garfqf.clear();//solo per exp garfield
  evento.tplast.clear();// tempo del plastichino (solo exp)
  evento.rcoqf.clear();//solo per exp RCo
  evento.rcocode.clear();
 ...
 for(int j=0;j<2;j++)
   {
	...
	mcevent.partbuona[j]=-1;
        ...
        mcevent.array[j]=-1; //per output kvsim
        mcevent.ntele[j]=-1;
        mcevent.idcode[j]=-1;
        mcevent.ecode[j]=-2;
...
if(Classe_analisi::Getanalisi()->tipo_analisi>=20)
  {
	...
	if(Classe_analisi::Getanalisi()->tipo_analisi==24){
		//printf("Sono qui %p\n",&evento);
		indrafazia.mtot=0;
		evento.moltepl=0;
                /*
                for(int jjj=0; jjj<1000;jjj++)
                 {
                  indrafazia.idtel[jjj]= -1;
                  indrafazia.blocco[jjj]= -1;
                  indrafazia.qua[jjj]= -1;
                  indrafazia.tel[jjj]= -1;
                  indrafazia.ring[jjj]= -1;
                  indrafazia.module[jjj]= -1;
                  indrafazia.array[jjj]= -1;
                  indrafazia.gt_dt[jjj]= -1;
                 }
                */
		return;
	}
...
void Classe_evento::Copia_in_Evento(int tipo_analisi)
{
  ...
  if(tipo_analisi<20)
    {
	  ...
	  for(unsigned j=0;j<mcevent.moltepl;j++)
	{
	  if(mcevent.partbuona[j]>0) // && mcevent.thetacmexp[j]<8)
	    {
		...
		evento.vplab.push_back(vjl);
	      ...
	      if(mcevent.Ameasured[j]>-1)//per output kvsim	      
	       {
	        if(mcevent.array[j]>-1) evento.codphos.push_back(mcevent.array[j]);	       
	        if(mcevent.ntele[j]>-1) evento.coderiv.push_back(mcevent.ntele[j]);
	        if(mcevent.idcode[j]>-1) evento.rcocode.push_back(mcevent.idcode[j]);
	        if(mcevent.ecode[j]>-2) evento.phosqf.push_back(mcevent.ecode[j]);
	        if(mcevent.Ameasured[j]>-1) evento.rcoqf.push_back(mcevent.Ameasured[j]);
	       }
  ...
  if(tipo_analisi>=20&& tipo_analisi<22)//EXP leggibile EXP compatto; uscite di ODIE
    {
...
evento.moltepl++;
    ...
    }//if(tipo_analisi>=20&&tipo_analisi<22)

  //ntupla di BOLOGNA***************
 if(tipo_analisi==22)//EXP Ntupla di Bologna
    {
...
evento.moltepl++;
  ...
  //ntupla di KALIVEDA per ISOFAZIA***************
 if(tipo_analisi==23)//EXP ntupla KALIVEDA per ISOFAZIA
    {
      ...
      if(Classe_geo::Getgeo()->ThereIsBlocchiFazia==1 && faziakali.mtot>0)
	{
	  ...
	  for(int j=0;j<faziakali.mtot;j++)
		...
		if(faziakali.idcode[j]==0 && faziakali.ecode[j]>=0&&faziakali.ecode[j]<3)//identificazione buona e energia buona
		{
...
evento.garfqf.push_back(-1);
 ...
 evento.tbunch.push_back(-1);
    ...
    }// fine tipo_analisi==23 ntupla di KALIVEDA per ISOFAZIA
 ...
 if(tipo_analisi==24)//EXP ntupla KALIVEDA per INDRAFAZIA
    {
	    ...
	    if(scarta==0)
	     {
	      ...
	      evento.thetalab.push_back(theout);
	      evento.philab.push_back(phiout);
	      float dout=100.; //----------------------------------------------------------ATTENZIONE: DISTANZA DEL RIVELATORE. SERVE SOLO PER IL TEMPO DI VOLO, CHE ORA È ERRATO!!!!!!! 
	      float vv=Classe_formule::e2v(evento.epartlab[evento.moltepl],evento.a[evento.moltepl]);
              evento.vpartlab.push_back(vv);
    ...
    }// fine tipo_analisi==24 ntupla di KALIVEDA per INDRAFAZIA
...
//vector<float> Classe_evento::erel2part(vector<float> epart1, vector<float> epart2)
vector<float> Classe_evento::erel2part(vector<vector<float> > mixingprel, int a1, int a2)
{
  vector <float> erel_vect;
  float ared=(float)a1*(float)a2/((float)a1+(float)a2);
    for(int i=0;i<mixingprel.size();i++)
     
    {
     

            erel_vect.push_back(pow(Classe_formule::modulo(mixingprel.at(i)),2)/(2*ared*Classe_formule::amu));
...
vector<vector <float> > Classe_evento::prel2part(vector<vector<float> > ppart1, vector<vector<float> > ppart2,int a1,int a2){
float ared=(float)a1*(float)a2/((float)a1+(float)a2);
...
vector<vector<float> > Classe_evento::pmix(int index, vector<vector<vector<vector<float> > > > tableP, vector<vector<float> > ppart,int a1,int a2){
float ared=(float)a1*(float)a2/((float)a1+(float)a2);
  ...
  for(UInt_t i=0; i<ppart.size(); i++){
	...
	for(UInt_t q=0; q<tableP.at(index).at(j).size(); q++){
	  if(!tableP.at(index).at(j).at(q).empty()){
	    vector<float> p_temp;
	    if(ppart.at(i).at(3)!=tableP.at(index).at(j).at(q).at(3)){ //la posizione 3 tiene il coderiv della particella! non voglio mixare particelle diverse ma viste dallo stesso rivelatore. avrei infatti piu' efficienza rispetto a quanto ho realmente
	    	for(int k=0; k<3; k++){
...
vector<float> Classe_evento::emix(vector<vector<float> > mixingprel, int a1, int a2)
{
  vector <float> erel_vect;
  float ared=(float)a1*(float)a2/((float)a1+(float)a2);
    for(int i=0;i<mixingprel.size();i++)
...
void Classe_evento::mixatore(int index, int n1, int z1, int a1, int n2, int z2, int a2)
{
...
if(n1>1&&n2==-1){
 ...
 for(UInt_t m=0; m<evento.moltepl; m++){
   if(TMath::Nint(evento.a[m])==a1&&TMath::Nint(evento.z[m])==z1) count++;
 }
	...
	Classe_analisi::Getanalisi()->jokmix.at(index)=Classe_analisi::Getanalisi()->iokmix.at(index)-Classe_analisi::Getanalisi()->ioktimesmix.at(index)*buffer;
	 ...
	 for(UInt_t m=0; m<evento.moltepl; m++){
	   if(TMath::Nint(evento.a[m])==a1&&TMath::Nint(evento.z[m])==z1) {
	
	     vector<float> ppart;
	     
	
	     ppart.push_back(evento.a[m]*Classe_formule::amu*evento.vpartcm_x[m]/Classe_formule::cluce);
	     ppart.push_back(evento.a[m]*Classe_formule::amu*evento.vpartcm_y[m]/Classe_formule::cluce);
	     ppart.push_back(evento.a[m]*Classe_formule::amu*evento.vpartcm_z[m]/Classe_formule::cluce);
	 ...
	 mixing.Prel.at(index) = prel2part(Ppart,Ppart,a1,a1);
	 mixing.Erel.at(index) = erel2part(mixing.Prel.at(index),a1,a1);
	 
	 
	 //Mi calcolo il mix con gli elementi messi nella tabella (primo giro no)
	 if(Classe_analisi::Getanalisi()->iokmix.at(index)>0) {
	   mixing.Pmix.at(index)=pmix(index,Classe_analisi::Getanalisi()->tableP1,Ppart,a1,a1);
	   mixing.Emix.at(index)=emix(mixing.Pmix.at(index),a1,a1);
	 }
...
//	 Classe_analisi::Getanalisi()->tableP1.at(index).resize(Classe_analisi::Getanalisi()->tableP1.at(index).size()+1);
     ...
     if(n1>0&&n2>0){
       ...
       for(UInt_t m=0; m<evento.moltepl; m++){
	 if(TMath::Nint(evento.a[m])==a1&&TMath::Nint(evento.z[m])==z1) count1++;
	 if(TMath::Nint(evento.a[m])==a2&&TMath::Nint(evento.z[m])==z2) count2++;
       }
       
             
       if(count1==n1&&count2==n2){//seleziono la fisica
	 if(Classe_analisi::Getanalisi()->iokmix.at(index)>0&&Classe_analisi::Getanalisi()->iokmix.at(index)%buffer==0) Classe_analisi::Getanalisi()->ioktimesmix.at(index)++;
	 Classe_analisi::Getanalisi()->jokmix.at(index)=Classe_analisi::Getanalisi()->iokmix.at(index)-Classe_analisi::Getanalisi()->ioktimesmix.at(index)*buffer;
	 ...
	 for(UInt_t m=0; m<evento.moltepl; m++){
	   if(TMath::Nint(evento.a[m])==a1&&TMath::Nint(evento.z[m])==z1) {
	     vector<float> ppart1;
	     ppart1.push_back(evento.a[m]*Classe_formule::amu*evento.vpartcm_x[m]/Classe_formule::cluce);
	     ppart1.push_back(evento.a[m]*Classe_formule::amu*evento.vpartcm_y[m]/Classe_formule::cluce);
	     ppart1.push_back(evento.a[m]*Classe_formule::amu*evento.vpartcm_z[m]/Classe_formule::cluce);
	   ...
	   if(TMath::Nint(evento.a[m])==a2&&TMath::Nint(evento.z[m])==z2) {
	     vector<float> ppart2;
	     ppart2.push_back(evento.a[m]*Classe_formule::amu*evento.vpartcm_x[m]/Classe_formule::cluce);
	     ppart2.push_back(evento.a[m]*Classe_formule::amu*evento.vpartcm_y[m]/Classe_formule::cluce);
	     ppart2.push_back(evento.a[m]*Classe_formule::amu*evento.vpartcm_z[m]/Classe_formule::cluce);
	...
	cout << "\nMIX-Ppart2:\n";
	   ...
	   for(int k=0; k<Ppart2.at(i).size(); k++){
	   cout << Ppart2.at(i).at(k) << " ";
	   }
	   }*/
	   
	 //calcolo Erel
	 mixing.Prel.at(index) = prel2part(Ppart1,Ppart2,a1,a2);	 
	 mixing.Erel.at(index) = erel2part(mixing.Prel.at(index),a1,a2);
	...
	for(int i=0; i<mixing.Prel.at(index).size(); i++){
	 ...
	 cout << " \n";*/
	   ...
	   mixing.Pmix.at(index)=pmix(index,Classe_analisi::Getanalisi()->tableP1,Ppart1,a1,a2);
	...
	/*   cout << "Table 2:\n";
	 ...
	 cout << " \n";*/
	   ...
	   pstep=pmix(index,Classe_analisi::Getanalisi()->tableP2,Ppart2,a2,a1);
	   mixing.Pmix.at(index).insert(mixing.Pmix.at(index).end(), pstep.begin(), pstep.end() );
	   mixing.Emix.at(index)=emix(mixing.Pmix.at(index),a1,a2);

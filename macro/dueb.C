   ...
   for(int j=0;j<nf;j++)
     {
	  ...
	  ggrec[j]=(TGraphErrors**)malloc(57*sizeof(TGraphErrors*));
       for(int iz=0;iz<57;iz++)
 	{
 	  ggrec[j][iz]=new TGraphErrors();
 	  ggrec[j][iz]->SetMarkerStyle(sym[j]);
 	  ggrec[j][iz]->SetMarkerColor(col[j]);
	  ggrec[j][iz]->SetLineColor(col[j]);
	  ggrec[j][iz]->SetMarkerSize(1.2);

	}

       //h[j]=(TH2F***)malloc(40*sizeof(TH2F**));
       for(int iz1=5;iz1<40;iz1++)
	 {
	   //h[j][iz1]=(TH2F**)malloc(40*sizeof(TH2F*));
	   for(int iz2=5;iz2<=iz1;iz2++)
	     {

	       int ix0=h1[j]->GetXaxis()->FindBin(iz1*100.);
	       int ix1=h1[j]->GetXaxis()->FindBin((iz1+1)*100.);
	       int iy0=h1[j]->GetYaxis()->FindBin(iz2*100.);
	       int iy1=h1[j]->GetYaxis()->FindBin((iz2+1)*100.);
	       //cout<<iz1<<" "<<iz2<<endl;
	       if(h1[j]->Integral(ix0,ix1,iy0,iy1)>0)
		 {
		   //  cout<<h1[j]->Integral(ix0,ix1,iy0,iy1)<<endl;
		   //       h[j][iz1][iz2]=new TH2F(Form("h%d_%d_%d",j,iz1,iz2),Form("h%d_%d_%d",j,iz1,iz2),h1[j]->GetXaxis()->GetNbins(),h1[j]->GetXaxis()->GetXmin(),h1[j]->GetXaxis()->GetXmax(),h1[j]->GetYaxis()->GetNbins(),h1[j]->GetYaxis()->GetXmin(),h1[j]->GetYaxis()->GetXmax());
		   //for(int ix=ix0;ix<ix1;ix++)
		   // {
		   //for(int iy=iy0;iy<iy1;iy++)
		   //  {
		   //   h[j][iz1][iz2]->SetBinContent(ix,iy,h1[j]->GetBinContent(ix,iy));
		   //  }
		   // }
		   // cout<<iz1<<" "<<iz2<<" "<<h[j][iz1][iz2]->GetCorrelationFactor(1,2)<<endl;
	       h1[j]->GetXaxis()->SetRange(ix0,ix1);
	       h1[j]->GetYaxis()->SetRange(iy0,iy1);
	       //cout<<"da h1 "<<h1[j]->GetCorrelationFactor(1,2)<<endl;

	       int izt=iz1+iz2;
	       float asym=(float)(iz1-iz2)/(iz1+iz2);
	       cout<<iz1<<" "<<iz2<<" "<<izt<<" "<<asym<<" "<<h1[j]->GetCorrelationFactor(1,2)<<endl;
	       ggrec[j][izt]->SetPoint(ggrec[j][izt]->GetN(),asym,h1[j]->GetCorrelationFactor(1,2));
	       ggrec[j][izt]->SetPointError(ggrec[j][izt]->GetN()-1,0.001,0.001);
...
// 	  ggqp[j]=(TGraphErrors**)malloc(57*sizeof(TGraphErrors*));
...
// 	  ggt2sub[j]=(TGraphErrors**)malloc(57*sizeof(TGraphErrors*));
       ...
       for(int iz=0;iz<57;iz++)
 	{
 // 	  ggqp[j][iz]=new TGraphErrors();
//  	  ggqp[j][iz]->SetMarkerStyle(sym[j]);
//  	  ggqp[j][iz]->SetMarkerColor(col[j]);
// 	  ggqp[j][iz]->SetLineColor(col[j]);
// 	  ggqp[j][iz]->SetMarkerSize(1.2);
...
// 	    {
// 	      i1=0;
// 	      double *x=ggqp[j][5]->GetX();
// 	  for(int k=0;k<ggqp[j][5]->GetN();k++)
// 	    {
// 	      if(x[k]<10)
// 		{
		 
// 		ggqp[j][5]->RemovePoint(k);
// 		i1=1;
// 		break;
// 		}
// 	    }
...
// 	    {
// 	      i1=0;
// 	      double *x=ggrec[j][5]->GetX();
// 	  for(int k=0;k<ggrec[j][5]->GetN();k++)
// 	    {
// 	      if(x[k]<10)
// 		{
		 
// 		ggrec[j][5]->RemovePoint(k);
// 		i1=1;
// 		break;
// 		}
// 	    }
...
// 	   double *x=ggqp[j][iz]->GetX();
//  	   ainf[j][iz]=1000;
//  	   asup[j][iz]=-1000;
// 	   for(int i=0;i<ggqp[j][iz]->GetN();i++)
// 	     {
...
// 	   double *x=ggrec[j][iz]->GetX();
// 	   double *y=ggrec[j][iz]->GetY();
//  	   ainfsub[j][iz]=1000;
//  	   asupsub[j][iz]=-1000;
// 	   for(int i=0;i<ggrec[j][iz]->GetN();i++)
// 	     {
...
// 	     }
	   ...
	   //cout<<iz<<" "<<am<<" "<<aM<<endl;
	   hf[iz]=new TH2F(Form("hf%d",iz),Form("Z=%d",iz),100,-0.1,1,100,-1.1,1.1);
	   ...
	   for(int k=0;k<nf;k++)
	       ...
	       if(!stit[k].Contains("stiff") &&!stit[k].Contains("soft"))
		 {
	       ...
	       //leg[iz]->AddEntry(ggqp[k][iz],Form("%sQP", tit[k]),"p");
	       leg[iz]->AddEntry(ggrec[k][iz],Form("%s", tit[k]),"p");
		 }
	     }
	   leg[iz]->Draw();
	 }
...
// //      }
   ...
   for(int j=1;j<16;j++)
	 ...
	 {
       hf[j+11]->Draw();
       hf[j+11]->GetYaxis()->SetRangeUser(-0.2,0.2);
	   for(int k=0;k<nf;k++)
	     {
	       if(!stit[k].Contains("stiff") &&!stit[k].Contains("soft"))
		 {
		   //   ggqp[k][j+11]->Draw("pl");
	       ggrec[k][j+11]->Draw("pl");
